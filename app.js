...
if(config.settings.database.type === 'embedded'){
    ...
    var bucket = cluster.openBucket(config.settings.database.bucket_name);
        ...
        bucket.upsert('user:king_arthur', {
            ...
            function (err, result) {
                bucket.get('user:king_arthur', function (err, result) {
                    ...
                    bucket.query(
                        N1qlQuery.fromString('SELECT * FROM ' + config.settings.database.bucket_name +  '  WHERE $1 in interests LIMIT 1'),
                        ['African Swallows'],
                        function (err, rows) {
